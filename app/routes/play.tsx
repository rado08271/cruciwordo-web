...
import {
  ...
  reducers,
  tables,
  WordPlacementsDatabaseModel,
} from "~/spacetime_bridge";
...
export function meta({ matches }: Route.MetaArgs) {
  ...
  const { boardModel, wordsModel } = matches.find(route => route && route.id === "routes/play")
  ...
  return [
    ...
    {
      name: "description",
      content: `Join a ${boardModel.rows}x${boardModel.cols} search puzzle with ${wordsModel.length} words on Cruciwordo. Collaborate with friends in real-time to find all words and unlock the hidden solution.`,
    },
    {
      name: "keywords",
      content:
        "play crossword, online word puzzle, multiplayer word search, collaborative puzzle game, word challenge",
    },
    ...
    { property: "og:title", content: "Play Puzzle | Cruciwordo Multiplayer Word Game" },
    {
      ...
      content:
        "Join a multiplayer word search puzzle on Cruciwordo. Collaborate with friends in real-time to find all words and unlock the hidden solution.",
    },
    {
      property: "og:image",
...
const Play = ({ params }: Route.ComponentProps) => {
  ...
  return <Suspense fallback={<Load />}>{/* <PlayLayout boardId={boardId}/> */}</Suspense>;
};
