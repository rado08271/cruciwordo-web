...
const SolutionList = ({ message, isSolved }: Props) => {
  const [wordSprings, wordsSpringsApi] = useSprings(
  ...
  return (
    <>
      ...
      <div
        className={
          "flex flex-row flex-wrap justify-center items-center gap-1 px-12 pb-6 md:p-2 text-stone-500"
        }
      >
        {message.split(" ").map((word, index) => {
          const spring = wordSprings.at(index);
          return (
            <animated.span style={spring} key={`${word}-${index}`}>
...
const Cell = ({ children, isSolved }: PropsWithChildren & NestedProp) => {
  return (
    <div
      className={
        "text-xs md:text-lg w-5 h-5 md:w-7 md:h-7 aspect-square bg-slate-50 border-b-2 border-stone-200 rounded-sm font-thin text-md flex justify-center items-center"
      }
    >
      {isSolved ? children : ""}
    </div>
  ...
  return (
    <div className={"flex flex-row flex-wrap justify-center items-center gap-1 px-4"}>
      {typeof children === "string" &&
        children.split("").map((cell, index) => {
          if (RegExp("[A-Za-z]").test(cell)) {
            return (
              <span key={`${children}-${cell}-${index}`}>
                <Cell isSolved={isSolved}>{cell}</Cell>
              </span>
            );
          } else {
            return (
              <div key={`${children}-${cell}-${index}`} className={"text-stone-300"}>
                {cell}
              </div>
            );
          }
